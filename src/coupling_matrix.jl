module coupling_matrix
    ...
    function HessianEquidistantTrap()
    ...
    function Hessian(u_ions::Array{Float64}, ω_trap::Array{Float64})
        Nions = size(u_ions)[2]; Nmodes = 3*Nions;
        ...
        for i in 1:Nions
            for j in 1:Nions
                if i!=j
                    ...
                    for α in 1:3
                        for β in 1:3
                            u = i + (α-1)*Nions; v = j + (β-1)*Nions;
                            ddVtilde = 0;
                            ddV = ((α==β)*(r_ij)^2 - 3*(u_ions[α,i]-u_ions[α,j])*(u_ions[β,i]-u_ions[β,j]))/((r_ij)^5);
                            Hess[u, v] = ddVtilde + ddV;
                        end
                    end
                elseif i==j
                    for α in 1:3
                        for β in 1:3
                            u = i + (α-1)*Nions; v = j + (β-1)*Nions;
                            ddVtilde = (ω_trap[α]/ω_trap[3])^2*(α==β);
                            ddV = sum([i!=k ? (-(α==β)*norm(u_ions[:,i]-u_ions[:,k])^2 + 3*(u_ions[α,i]-u_ions[α,k])*(u_ions[β,i]-u_ions[β,k]))/(norm(u_ions[:,i]-u_ions[:,k])^5) : 0 for k in 1:Nions]);
                            Hess[u, v] = ddVtilde + ddV;
                        end
                    end
                end
    ...
    ### Hessian unpinned system homogeneous spacing
    function Hessian2(u_ions::Array{Float64}, ω_trap::Array{Float64}, coeffs::Array{Float64}, size_crystal::Float64)
        Nions = size(u_ions)[2]; Nmodes = 3*Nions;
        x0=size_crystal; d=2*x0/Nions; d += d/(Nions -1);
        Hess  = zeros(Nmodes, Nmodes);
        #global (a,b,e = coeffs[1], coeffs[2], ee)
        HessTrapEqui = HessianEquidistantTrap()
        for α in 1:3, β in 1:3, i in 1:Nions, j in 1:Nions
            u = i + (α-1)*Nions; v = j + (β-1)*Nions;
            if i!=j
                r_ij = norm(u_ions[:,i]-u_ions[:,j]);
                ddVharmonic = 0;
    ...
    function Jexp1D(pos_ion::Array, Ωpin::Vector, ω_trap::Vector, μ_raman::Float64; kvec::Vector=[0,1,0]) #Working. Too many calls to eigen!
    ...
    function Jexp3D(pos_ion::Array{Float64,2}, Ωpin::Array{Float64,2}, ω_trap::Array{Float64,1}, μ_raman::Float64; kvec::Vector=[0,1,0]) #Working!
        Nions = size(pos_ion)[2];
        Nmodes = 3*Nions;
        l0 = (ee^2/(4*π*ϵ0*mYb*ω_trap[3]^2))^(1/3);
        u_ions = pos_ion#/l0;
