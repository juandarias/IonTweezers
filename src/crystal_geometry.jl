module crystal_geometry
    ...
    function PositionIons(Nions::Int64, ω_trap::Array{Float64,1}; plot_position::Bool=false, cvel::Float64=10E-19, tcool::Float64=100E-6)
        Ω=ω_trap; 
        cel=ee^2/(4*π*ϵ0)/mYb;
        x0=.4E-2;
        l0 = (ee^2/(4*π*ϵ0*mYb*ω_trap[3]^2))^(1/3);
        u0=(rand(3,Nions) .- .5)*x0; du0=zeros(3,Nions);
        tspan = (0.0,tcool);
        function diffeqs2!(ddu,du,u,p,t)
           for a in 1:Nions
              for dir in 1:3
    ...
    function PositionIons(Nions::Int64, ω_trap::Array{Float64,1}, size_crystal::Float64, coeffs::Array{Float64}; plot_position::Bool=false, cvel::Float64=3E-20, tcool::Float64=200E-6)
      Ω=ω_trap;
      cel=ee^2/(4*π*ϵ0*mYb);
      x0=size_crystal; d=2*x0/Nions; d += d/(Nions -1);
      l0=(ee^2/(4*π*ϵ0*mYb*ω_trap[3]^2))^(1/3);
      u0=x0*rand(-1.0:1e-6:1.0,(3,Nions)); du0=zeros(3,Nions); tspan = (0.0,tcool);
