...
function TransverseFieldIsing(J::Array{Float64, 2}, h::Vector{Float64})
    N_sites = size(J)[2]
    length(h) == 1 || length(h) == N_sites || throw(ArgumentError("h must be either length 1 or length $N_sites"))
    length(h) == 1 && (h = fill(h[1], N_sites))
    ...
    for input_ind in 0:2^N_sites-1 #input-ind -> input state index
        ...
        @inbounds for site in 1:N_sites
            @inbounds for next_site in site+1:N_sites
                bond_J = (input_bits[site] == input_bits[next_site]) ? J[site,next_site] : -J[site,next_site]
                nz_val += bond_J
            end
        end
        push!(nz_vals, nz_val)
    end
    return TransverseFieldIsing{2}((N_sites,N_sites), h, J, row_ixs, col_ixs, nz_vals)
end
